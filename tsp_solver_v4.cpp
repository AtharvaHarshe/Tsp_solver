                ...
                const vector<vector<int>>& cand)
{
    ...
    for (int idx = 0; idx < n; idx++) {
        ...
        for (int b : cand[a]) {
            ...
            if (nw + 1e-12 < old) {
                // reverse segment between idx+1 and j (cyclic-safe)
                if (idx < j)
...
bool orOptLimited(vector<int>& T, const vector<vector<double>>& dist,
                  int MAX_MOVES = 2000)
{
    int n = (int)T.size();
    bool improved = false;
...
// ---------------- Strong local search: repeat 2-opt + Or-opt ----------------
void localSearch(vector<int>& T,
                 const vector<vector<double>>& dist,
                 const vector<vector<int>>& cand)
{
    while (true) {
        bool improved = false;
        if (twoOptFast(T, dist, cand)) improved = true;
        if (orOptLimited(T, dist, 2000)) improved = true;
        if (!improved) break;
    }
}
...
int main() {
    ...
    // -------- Initial tour from best-of-40 NN --------
    vector<int> best = bestNN(dist, 40);
    localSearch(best, dist, candList);

    double bestL = tourLen(best, dist);
    cout << "Initial tour length after LS: " << bestL << "\n";

    // -------- ILS + Multi-start Loop (up to 55 seconds) --------
    const double LIMIT = 55.0;
    ...
    while (true) {
        ...
        vector<int> T;
        // every 3rd iteration: completely random start (multi-start)
        if (iter % 3 == 0) {
            T.resize(N);
            iota(T.begin(), T.end(), 0);
            shuffle(T.begin(), T.end(), rng);
        } else {
            // otherwise: perturb best solution (ILS)
            T = best;
            doubleBridge(T);
        }

        localSearch(T, dist, candList);
        double L = tourLen(T, dist);

        if (L < bestL) {
            bestL = L;
            best = T;
            double nowT = chrono::duration<double>(clock_tp::now() - t0).count();
            cout << "Improved: " << L << "  at t=" << nowT << " sec\n";
        }
